echo "set ttyMU0 and ttyMU1 as RS485 half duplex and set baud rate as 115200"
sudo uartmode -p 0 -m 1
sudo uartmode -p 1 -m 1
...
if [ `cat ttyDumpMU1.dat | grep -c send-to-MU0` -eq 1 ]; then
    if [ `cat ttyDumpMU0.dat | grep -c send-to-MU1` -eq 1 ]; then
        echo "RS485 half deplux test success!"
    else
        echo "RS485 half deplux test fail!"
    fi
else
    echo "RS485 half deplux test fail!"
fi
